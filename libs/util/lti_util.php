...
class BLTI {
    ...
    function getUserResultSourceId() {
        $sourcedid = $this->info['lis_result_sourcedid'];
        if (get_magic_quotes_gpc()) $sourcedid = stripslashes($sourcedid);
        if ( strlen($sourcedid) > 0 ) return $sourcedid;
        return false;        
    }
    ...
    function getResourceGradeURL() {
        $urlbasic = $this->info['ext_ims_lis_basic_outcome_url'];
        $url = $this->info['lis_outcome_service_url'];
        if ( strlen($url) > 0 ) return $url;
        if ( strlen($urlbasic) > 0) return $urlbasic;
        return false;           
    }
    
    function isReourceGradeBasic() {
        $urlbasic = $this->info['ext_ims_lis_basic_outcome_url'];
        $url = $this->info['lis_outcome_service_url'];
        if ( strlen($url) > 0 ) return false;
        if ( strlen($urlbasic) > 0) return true;
        return false;           
    }
    ...
    function getResourceUniqueId() {
        $id = $this->info['resource_unique_id'];
        if ( strlen($id) > 0 ) return $id;
        return false;        
    }
    ...
    function getInstanceID() {
        $instance_guid = $this->info['tool_consumer_instance_guid'];
        $instance_url = parse_url($this->info['lis_outcome_service_url'], PHP_URL_HOST);
    if ( strlen($instance_guid) >  0) return $instance_guid;
    if ( strlen($instance_url) >  0) return $instance_url;
    return false;
    }    
    ...
    function getInstanceName() {
        $instance_name = $this->info['tool_consumer_instance_name'];
    if ( strlen($instance_name) >  0) return $instance_name;
    return false;
    }   
    ...
    function dump() {
        ...
        $ret .= "getInstanceID() = ".$this->getInstanceID()."\n";
        $ret .= "getInstanceName() = ".$this->getInstanceName()."\n";     
        $ret .= "getInstanceURL() = ".$this->getInstanceURL()."\n";             
